...
def get_csv_url(url: String) : List[String] = {
    ...
    val allDataLines = bufferedSource.getLines().drop(1).toList
...
def divide_csv_lines(lines: List[String]): List[List[String]] = {
    for (line <- lines) yield line.split(",").map(_.trim).toList
}

def process_ratings(lines: List[String]) : List[(String, String)] = {
    val lst = divide_csv_lines(lines)
    val temp = lst.filter(x => (x(2).toInt >= 4))
    for(n <- temp) yield ((n(0).toString, n(1).toString))
}

def process_movies(lines: List[String]) : List[(String, String)] = {
    val lst = divide_csv_lines(lines)
    for(n <- lst) yield ((n(0).toString, n(1).toString))
}
...
def groupByIdRecursively(ratings: List[(String, String)], m: Map[String, List[String]]) : Map[String, List[String]] = ratings match {
    case Nil => m
    case head :: tail => {
        val id = head._1
        val temp = ratings.filter(x => x._1 == id)
        val a = temp.groupBy(_._1).view.mapValues(_.map(_._2)).toMap
        groupByIdRecursively(ratings.takeRight(ratings.length - temp.length), m ++ a)
    }
}

def groupById(ratings: List[(String, String)], m: Map[String, List[String]]) : Map[String, List[String]] = {
    val lm = groupByIdRecursively(ratings.sortBy(_._1), m)
    lm
}
...
def remove(movies: List[String], mov: String) = movies diff List(mov)

def favourites(m: Map[String, List[String]], mov: String) : List[List[String]] = {
    val valuesList = m.values.toList
    val recommendations: List[List[String]] = for(x <- valuesList; if x.contains(mov)) yield x
    for(x <- recommendations) yield remove(x, mov)
}
...
def suggestions(recs: Map[String, List[String]], mov_name: String) : List[String] = {
    val allMovies = favourites(recs, mov_name).flatten
    val frequency = allMovies.groupBy(el => el).map(e => (e._1, e._2.length))
    (frequency.toSeq.sortBy(_._2)).map(_._1).toList.reverse
}
...
def recommendations(recs: Map[String, List[String]], movs: Map[String, String], mov_name: String) : List[String] = {
    val most_frequent = suggestions(recs, mov_name)
    if(most_frequent == Nil || most_frequent.length == 0) {
        Nil
    }
    else if(most_frequent.length == 1) {
        List(movs(most_frequent(0)))
    }
    else {
        List(movs(most_frequent(0)), movs(most_frequent(1)))
    }
}
