...
def jtable(pg: String) : Map[Int, Int] = {
    val starts = for(i <- 0 until pg.length; if (pg.charAt(i) == '[')) yield i
    val startsFinishes = for(n <- starts) yield (n, jumpRight(pg, n+1,0))
    val lst = for(n <- startsFinishes) yield (n._2 - 1) -> (n._1 + 1)
    val merged = (startsFinishes.++(lst)).toMap
    merged.toMap
}
...
def compute2(pg: String, tb: Map[Int, Int], pc: Int, mp: Int, mem: Mem) : Mem = {
    if(pc < 0 || pc == pg.length) {
        ...
        pg.charAt(pc) match {
            ...
            case '<' => compute2(pg, tb, pc+1, mp-1, mem)
            case '+' => {
                val valueAtPointer = sread(mem, mp)
                compute2(pg, tb, pc+1, mp, write(mem, mp, valueAtPointer+1))
            }
            case '-' => {
                val valueAtPointer = sread(mem, mp)
                compute2(pg, tb, pc+1, mp, write(mem, mp, valueAtPointer-1))
...
def compute3(pg: String, tb: Map[Int, Int], pc: Int, mp: Int, mem: Mem) : Mem = {
    if(pc < 0 || pc == pg.length) {
        ...
        pg.charAt(pc) match {
            ...
            case '+' => {
                val valueAtPointer = sread(mem, mp)
                compute3(pg, tb, pc+1, mp, write(mem, mp, valueAtPointer+1))
            }
            case '-' => {
                val valueAtPointer = sread(mem, mp)
                compute3(pg, tb, pc+1, mp, write(mem, mp, valueAtPointer-1))
