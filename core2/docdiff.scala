...
def clean(s: String) : List[String] = {
    ...
    regularExpression.findAllIn(s).toList
}
...
def occurrences(xs: List[String]): Map[String, Int] = {
    xs.groupBy(el => el).map(e => (e._1, e._2.length))
}
...
def getListOfProducts(lst1: List[String], lst2: List[String]) : List[Int] = {
    if(lst1.distinct.length > lst2.distinct.length) {
        for(n <- lst1.distinct) yield(if(!occurrences(lst2).contains(n)) 0 else (occurrences(lst1).get(n).get * occurrences(lst2).get(n).get))
    }
    else {
        for(n <- lst2.distinct) yield(if(!occurrences(lst1).contains(n)) 0 else (occurrences(lst2).get(n).get * occurrences(lst1).get(n).get))
    }
}

def prod(lst1: List[String], lst2: List[String]) : Int = getListOfProducts(lst1, lst2).sum
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    val d1: Double = prod(lst1, lst1)
    val d2: Double = prod(lst2, lst2)
    if(d1 > d2) {
        val a: Double = prod(lst1, lst2) / d1
        a
    }
    else {
        val a: Double = prod(lst1, lst2) / d2
        a
    }
}

def similarity(s1: String, s2: String) : Double = {
    val a: Double = overlap(clean(s1), clean(s2))
    a
}
